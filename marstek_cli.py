...
def safe_json(data: bytes) -> dict:
    try:
        ...
        return {"error": "Ungültige JSON-Antwort vom Gerät"}
...
def query(ip: str, port: int, method: str, params: dict = None) -> dict:
    ...
    for attempt in range(1, RETRIES + 1):
        try:
            ...
            last_error = f"Timeout nach {RETRIES} Versuchen"
            if attempt < RETRIES:
...
def discover_devices(port: int, timeout: float = 3.0) -> list[dict]:
...
def fmt_error(r: dict) -> str:
    if r.get("skipped"):
        return f"  (nicht unterstützt — {r['error']})"
...
def show_device(ip, port):
    section("Gerät — Marstek.GetDevice")
    r = query(ip, port, "Marstek.GetDevice", {"ble_mac": "0"})
    if not ok_result(r):
        return
    print(fmt("Modell",      r.get("device")))
    ...
    print(fmt("IP-Adresse",  r.get("ip")))


def show_wifi(ip, port):
    section("WLAN — Wifi.GetStatus")
    r = query(ip, port, "Wifi.GetStatus")
    if not ok_result(r):
        return
    print(fmt("SSID",         r.get("ssid")))
    print(fmt("IP-Adresse",   r.get("sta_ip")))
    print(fmt("Gateway",      r.get("sta_gate")))
    print(fmt("Subnetzmaske", r.get("sta_mask")))
...
def show_ble(ip, port):
    ...
    if not ok_result(r):
        return
    print(fmt("Status",  r.get("state")))
    print(fmt("BLE MAC", r.get("ble_mac")))


def show_bat(ip, port):
    section("Batterie — Bat.GetStatus")
    r = query(ip, port, "Bat.GetStatus")
    if not ok_result(r):
        return
    print(fmt("SOC",                r.get("soc"), "%"))
    print(fmt_wh("Verbleibend",     r.get("bat_capacity")))
    print(fmt_wh("Nennkapazität",   r.get("rated_capacity")))
    print(fmt("Temperatur",         r.get("bat_temp"), "°C"))
    print(fmt("Laden erlaubt",      to_bool(r.get("charg_flag"))))
    print(fmt("Entladen erlaubt",   to_bool(r.get("dischrg_flag"))))
...
def show_es(ip, port):
    ...
    print(fmt_wh("Batteriekapazität",           r.get("bat_cap")))
    print(fmt("Batterieleistung",               r.get("bat_power"), "W"))
    print(fmt("PV-Leistung",                    r.get("pv_power"), "W"))
    print(fmt("Netz (+ Bezug / − Einspeisung)", r.get("ongrid_power"), "W"))
    print(fmt("Offgrid-Leistung",               r.get("offgrid_power"), "W"))
    print()
    print(fmt_wh("Gesamt PV erzeugt",           r.get("total_pv_energy")))
    print(fmt_wh("Gesamt Netzeinspeisung",       r.get("total_grid_output_energy")))
    print(fmt_wh("Gesamt Netzbezug",            r.get("total_grid_input_energy")))
    print(fmt_wh("Gesamt Last/Offgrid",         r.get("total_load_energy")))


def show_em(ip, port):
    section("Energiezähler — EM.GetStatus")
    r = query(ip, port, "EM.GetStatus")
    if not ok_result(r):
        return
    ct = r.get("ct_state")
    print(fmt("CT verbunden",   "Ja" if ct == 1 else "Nein"))
    print(fmt("Phase A",        r.get("a_power"), "W"))
    print(fmt("Phase B",        r.get("b_power"), "W"))
    print(fmt("Phase C",        r.get("c_power"), "W"))
    print(fmt("Gesamtleistung", r.get("total_power"), "W"))
    ie, oe = r.get("input_energy"), r.get("output_energy")
    if ie is not None or oe is not None:
        print()
        print(fmt_wh("Kumulierter Eingang (CT)", ie, 0.1))
        print(fmt_wh("Kumulierter Ausgang (CT)", oe, 0.1))


def show_mode(ip, port):
    section("Betriebsmodus — ES.GetMode")
    r = query(ip, port, "ES.GetMode")
    if not ok_result(r):
        return
    print(fmt("Modus",        r.get("mode")))
    print(fmt("SOC",          r.get("bat_soc"), "%"))
    print(fmt("Netzleistung", r.get("ongrid_power"), "W"))
    print(fmt("Offgrid",      r.get("offgrid_power"), "W"))
    ct = r.get("ct_state")
    if ct is not None:
        print()
        print(fmt("CT verbunden",  "Ja" if ct == 1 else "Nein"))
        ...
        print(fmt("Gesamt (CT)",   r.get("total_power"), "W"))
    ie, oe = r.get("input_energy"), r.get("output_energy")
    if ie is not None or oe is not None:
        print()
        print(fmt_wh("Kumulierter Eingang", ie, 0.1))
        print(fmt_wh("Kumulierter Ausgang", oe, 0.1))
...
def set_mode(ip, port, mode: str, power: int | None):
    MODES = {"auto": "Auto", "ai": "AI", "ups": "Ups", "passive": "Passive"}
    mode_key = mode.lower()
    if mode_key not in MODES:
        print(f"  Fehler: Ungültiger Modus '{mode}'. Gültig: auto | ai | ups | passive")
        return
    if not confirm("⚠️  Gerätemodus ändern?"):
        return
    api_mode = MODES[mode_key]
    section(f"Modus setzen → {api_mode}")
    if api_mode == "Passive":
        if power is None:
            print("  Fehler: --power <W> ist für Passive-Modus erforderlich")
            return
    ...
    if not ok_result(r):
        return
    print(fmt("Ergebnis", "✓ Erfolgreich" if to_bool(r.get("set_result")) else "✗ Fehlgeschlagen"))
    if api_mode == "Passive":
        print(fmt("Sollleistung", power, "W"))


def set_dod(ip, port, value: int):
    if not 30 <= value <= 88:
        print(f"  Fehler: DOD muss zwischen 30 und 88 liegen (angegeben: {value})")
        return
    if not confirm("⚠️  Entladetiefe ändern?"):
        return
    section(f"Entladetiefe — DOD.SET → {value}%")
...
def set_led(ip, port, state: str):
    ...
    label = "ein" if val else "aus"
    section(f"LED — Led.Ctrl → {label}")
...
def set_ble(ip, port, state: str):
    ...
    label = "gesperrt" if val else "aktiv"
...
def main():
    ...
    if args.ip is None:
        ...
        if not devices:
            print("  Keine Geräte gefunden. Ist die Open API in der App aktiviert?")
            print()
            return
        ...
        if len(devices) == 1:
            ...
            try:
                idx    = int(input("  Gerät auswählen [1]: ").strip() or "1") - 1
                chosen = devices[idx]
